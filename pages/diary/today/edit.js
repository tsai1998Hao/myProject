...
const getPetData = async (pet_id) => {
  ...
  const handleSubmit = async (e) => {
    ...
    const res = await axios.post(
      'http://localhost:3002/diary/today/edit',
      diary
    )

    if (res.data.status === 'success') {
      toast.success('資訊 - 寵物日記修改成功')
    } else {
      toast.error(`錯誤 - 寵物日記修改失敗`)
    }
  ...
  const handleDelete = async (e) => {
    const query = {
      diary_id: router.query.diary_id,
    }
    const res = await axios.post(
      'http://localhost:3002/diary/today/delete',
      query
    )

    if (res.data.status === 'success') {
      toast.success('資訊 - 寵物日記刪除成功')
    } else {
      toast.error(`錯誤 - 寵物日記刪除失敗`)
    }
    setTimeout(() => {
      router.push(`/diary/today?pet_id=${pet_id}`)
    }, 2000)
  ...
  const getDiary = async (diary_id) => {
    const res = await axios.get(
      `http://localhost:3002/diary/today/list/${diary_id}`
    )

    if (res.data.status === 'success') {
      // 設定獲取項目
      setDiary(res.data.data.diary)
      setCate(res.data.data.diary.act_category)
    }
  }
  ...
  useEffect(() => {
    if (router.isReady) {
      ...
      const { pet_id, diary_id } = router.query
      setPetId(pet_id)
